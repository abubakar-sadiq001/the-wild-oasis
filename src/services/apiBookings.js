...
export async function getBookings({ filter, sortBy, page }) {
  ...
  // FILTER
  if (filter)
    // down below I'm telling supabase to filter the data based on selected user value.
    query = query[filter?.method || "eq"](filter?.field, filter?.value);

  // SORT
  if (sortBy)
    // down below I'm telling supabase to sort the data based on selected user value.
